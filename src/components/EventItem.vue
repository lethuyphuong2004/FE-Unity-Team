<template>
  ...
  <div v-if="loading" class="rounded-2xl p-6 mb-6 bg-gradient-to-br from-gray-50 to-gray-100 animate-pulse overflow-hidden relative">
    <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/50 to-transparent -skew-x-12 animate-shine"/>
  ...
  >
    ...
    <div class="relative z-10">
      <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-2">
        <h3 class="text-gray-900 font-bold text-xl leading-tight group-hover:text-indigo-600 transition-colors bg-clip-text bg-gradient-to-r from-gray-800 to-gray-900">
          {{ title }}
        </h3>
        <div class="flex items-center" v-if="!(status === 'done' && joined)">
          <span v-if="status === 'done'" class="text-emerald-600 flex items-center"></span>
          <span v-else-if="dueDate" class="text-rose-600 flex items-center bg-rose-50/80 px-3 py-1.5 rounded-full text-sm backdrop-blur-sm border border-rose-100">
      ...
      <!-- Địa điểm nếu có -->
      <div v-if="location" class="text-gray-600 mb-4 flex items-center text-sm bg-gray-50/80 px-3 py-1.5 rounded-full w-max border border-gray-100 shadow-sm">
      ...
      <div class="flex justify-between items-center">
        <!-- Phần thưởng với gradient -->
        <div class="text-amber-700 flex items-center px-4 py-2 bg-gradient-to-br from-amber-50 to-amber-100 rounded-full text-sm font-medium border border-amber-100 shadow-sm">
          <svg v-if="rewardType === 'points'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1.5" viewBox="0 0 20 20" fill="currentColor">
            <path
        ...
        <div v-if="joined !== undefined && !(status === 'done' && joined)">
          <span v-if="joined" class="text-emerald-600 flex items-center bg-emerald-50/80 px-3 py-1.5 rounded-full text-sm border border-emerald-100 shadow-sm">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" viewBox="0 0 20 20" fill="currentColor">
              <path
                fill-rule="evenodd"
          ...
          <span v-else class="text-gray-500 text-sm bg-gray-50/80 px-3 py-1.5 rounded-full border border-gray-100 shadow-sm">Not Joined</span>
        </div>
      </div>

      <!-- Nút mua vé với hiệu ứng hover đẹp -->
      <div v-if="ticketPurchased === false" class="mt-5 text-right">
        <button
...
export default {
  ...
  props: {
    ...
    post: { // Thêm prop post để sử dụng post.joined
      type: Object,
      default: () => ({ joined: false })
    }
  },
  computed: {
    rewardType() {
